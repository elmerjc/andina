...
class report_ferrocarriles_wizard(osv.osv_memory):
    _name = 'wizard.report.ferrocarriles'
    _description = 'Impresion de Vagones y Patio Simon del control de lavado industrial'
    _columns = {
    ...
    def _check_change_date_presentacion(self):
        if self.date_presentacion:
            ...
            str_titulo = "LAVANDERIA ROPA DE TRABAJO VAGONES, PATIO SIMON " + str(self.month_name(mes_number)) + " " + str(anio)
    ...
    def print_report_ferrocarriles(self,cr,uid,ids,context=None):
        ...
        return self.pool['report'].get_action(cr,uid,[],'andina_industrial.report_ferrocarriles',data=data,context=context)