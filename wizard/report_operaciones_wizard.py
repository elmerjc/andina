...
class report_operaciones_wizard(osv.osv_memory):
    _name = 'wizard.report.operaciones'
    _description = 'Impresion del operaciones del control de lavado industrial'
    _columns = {
    ...
    def _check_change_date_presentacion(self):
        if self.date_presentacion:
            ...
            str_titulo = "LAVANDERIA ROPA DE TRABAJO OPERACIONES " + str(self.month_name(mes_number)) + " " + str(anio)
    ...
    def print_report_operaciones(self,cr,uid,ids,context=None):
        ...
        return self.pool['report'].get_action(cr,uid,[],'andina_industrial.report_operaciones',data=data,context=context)