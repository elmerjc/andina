...
class report_cantidades_wizard(osv.osv_memory):
    _name = 'wizard.report.cantidades'
    _description = 'Impresion del reporte del control de lavado industrial'
    _columns = {
    ...
    @api.onchange('gerencia_id')
    def _check_change_gerencia_id(self):
        if self.date_presentacion:
            s = self.date_presentacion
            slist = s.split("-")
            sdate = datetime.date(int(slist[0]),int(slist[1]),int(slist[2]))
    ...
    def print_report_cantidades(self,cr,uid,ids,context=None):
        ...
        return self.pool['report'].get_action(cr,uid,[],'andina_industrial.report_cantidades',data=data,context=context)